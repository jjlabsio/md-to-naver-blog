...
`mtnb.dev` 배포에는 네이버 서치어드바이저 사이트 검증을 위한 환경 변수가 필요합니다.

| 변수                      | 설명                                                     |
| ------------------------- | -------------------------------------------------------- |
| `NAVER_SITE_VERIFICATION` | `<meta name="naver-site-verification" />`의 `content` 값 |

웹 앱은 T3 Env로 환경 변수를 검증하므로 이 값이 비어 있거나 설정되지 않으면 빌드/실행 시 실패합니다. 페이지는 정적으로 생성되므로 배포 플랫폼에 이 값을 설정한 뒤 다시 배포해야 메타태그에 반영됩니다.

CI와 릴리스 워크플로우는 빌드 검증용 placeholder 값을 사용합니다. 실제 `mtnb.dev` 배포 환경에는 네이버 서치어드바이저에서 발급받은 값을 설정해야 합니다.
