...
```ts
type ComponentRenderer = (props: ComponentProps, children: string) => string;
```

- `props`: 컴포넌트에 전달된 속성 (`{ [key: string]: string }`)
- `children`: 인라인 마크다운이 처리된 HTML 문자열 (self-closing이면 빈 문자열)

## 제한사항

- **인라인 CSS만 사용**: 네이버 블로그 에디터가 CSS class를 지원하지 않아 모든 스타일이 인라인으로 적용됩니다
- **지원하지 않는 문법**: footnote, math(LaTeX), 정의 리스트, task list(체크박스)
- **런타임 의존성**: [gray-matter](https://github.com/jonschlinkert/gray-matter) (frontmatter 파싱) 1개
